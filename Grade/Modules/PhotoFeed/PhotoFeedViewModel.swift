...
class PhotoFeedViewModel {
    ...
    fileprivate func loopVideo(_ asset: AVAsset) {
        let loopURL = filesPath.appendingPathComponent("loop.m4v")
        unlink(loopURL.path)
        
        VideoUtils.loopAsset(asset) { [weak self] reversedAsset in
            print(reversedAsset)
        }
    }
    ...
    func didSelectAsset(_ asset: PHAsset) {
        ...
        PHAssetResourceManager.default()
            .writeData(for: resources[1], toFile: actualUrl, options: nil) { [weak self] error in
                ...
                self?.loopVideo(AVAsset(url: actualUrl))
        }
    }
